...
SELECT s.user_id,ELT(MOD(n.n-1,8)+1,'Library Membership','Book Reservation','Lost Library Card','Digital Library Access','Borrowing Limit','Fine Payment','Reference Section','Library Timing'),ELT(MOD(n.n-1,8)+1,'I would like to know the process for renewing my library membership.','Can this book be reserved when it becomes available?','I have misplaced my library card. Please let me know how to obtain a replacement.','I am unable to access the digital library from outside the campus network.','How many books can a student borrow at one time?','Please explain the process for paying overdue fines.','Can reference books be borrowed overnight for academic work?','Will the library remain open during the upcoming examination period?'),ELT(MOD(n.n-1,3)+1,'OPEN','IN_PROGRESS','CLOSED'),DATE_SUB(NOW(),INTERVAL n.n*6 DAY)
FROM n JOIN students s ON s.rn=MOD(n.n-1,25)+1 WHERE NOT EXISTS (SELECT 1 FROM enquiry e WHERE e.user_id=s.user_id AND e.subject=ELT(MOD(n.n-1,8)+1,'Library Membership','Book Reservation','Lost Library Card','Digital Library Access','Borrowing Limit','Fine Payment','Reference Section','Library Timing') AND DATE(e.created_at)=DATE_SUB(CURDATE(),INTERVAL n.n*6 DAY));